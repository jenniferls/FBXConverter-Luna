...
void Loader::PrintData(const char* filePath) {
	...
	if (reader.readFile(filePath)){
		std::cout << "File was imported!!" << std::endl;
		...
		if (scene.hasCamera) {
			...
			std::cout << "Far plane: " << cam.camFar << std::endl;
			std::cout << "Near plane: " << cam.camNear << std::endl;
	...
	{
		std::cout << "File was NOT imported! FAILURE." << std::endl;
	}
}
